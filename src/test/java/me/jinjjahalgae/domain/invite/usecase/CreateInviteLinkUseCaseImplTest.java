...
class CreateInviteLinkUseCaseImplTest {
    ...
    void createNewInviteLink_Success() {
        // given
        Long contractId = 1L;
        User user = InviteTestUtil.createUser(1L, "userA", "userA_nick");
        Contract contract = InviteTestUtil.createContract(contractId, user);
        String contractKey = "contract:invite:" + contractId;

        when(contractRepository.findByIdWithUser(contractId)).thenReturn(Optional.of(contract));
        when(valueOperations.get(contractKey)).thenReturn(null); // 기존 링크 없음

        // when
        InviteLinkResponse result = createInviteLinkUseCase.createInviteLink(contractId, user);
