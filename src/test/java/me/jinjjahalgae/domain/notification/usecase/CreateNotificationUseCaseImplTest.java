...
import me.jinjjahalgae.domain.participation.usecase.get.validinfo.GetValidParticipantInfoUseCaseImpl;
...
class CreateNotificationUseCaseImplTest {
    ...
    @Mock
    private GetValidParticipantInfoUseCaseImpl getParticipantInfoByContractId;
    ...
    void SUPERVISOR_ADDED_타입으로_알림을_정상적으로_생성한다() {
        ...
        // Mock 설정
        when(getMyInfo.getMyInfo(actorUserId)).thenReturn(actorInfo);
        when(contractRepository.findContractById(contractId)).thenReturn(Optional.of(contract));
        when(getParticipantInfoByContractId.getValidParticipationInfo(contractId)).thenReturn(participants);
        ...
        createNotificationUseCase.createNotification(request);
